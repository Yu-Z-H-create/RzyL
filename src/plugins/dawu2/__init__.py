...
async def send_keyword_images(keywords: list[str], prefix: str = ""):
    ...
    if messages:
        ...
        await dawu2.finish(Message(messages))
    else:
        await dawu2.finish(f"{prefix}: {', '.join(keywords)}，但所有图片文件都不存在")
...
async def _(
    ...
    if not validate_input(name.result):
        await dawu2.finish("输入无效，请提供有效的实验名称（1-100个字符）")
        return

    if not check_rate_limit(event.user_id):
        await dawu2.finish("请求过于频繁，请稍后再试（每分钟最多10次请求）")
        return
