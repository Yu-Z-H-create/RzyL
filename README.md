...
EASTER_EGG_CHAT_PROMPT=                       # 彩蛋对话回退的预设系统提示词，留空使用内置默认值
```
...
1. **精确匹配**：按别名子串匹配个体与复合体，命中则出图（含随机文本）。
...
- 预设提示词由 `EASTER_EGG_CHAT_PROMPT` 配置，留空则使用内置默认值；`MODEL_CHAT` 留空时对话回退不可用（会提示「对话服务未配置」）。
- AI 路径（模糊匹配 + 对话回退）按用户限流，每 60 秒最多 10 次。
